...
| **History** | virtualized log graph, ref chips (branch/tag/HEAD), filter (message/author/hash) + go-to-hash, right-click actions (checkout, new branch/tag here, reword, cherry-pick, revert, reset soft/mixed/hard, rebase from here, copy hash) |
| **Diff** | unified & side-by-side, ignore-whitespace, prev/next change, per-hunk stage/unstage/revert, blame/annotate |
...
| **GitHub** | clone, pull requests (list / checkout / open in browser) via the `gh` CLI |
...
actions, side-by-side diff, hunk staging, 3-way conflict resolution, rebase with
resume, remotes, stash/reflog/submodules, a git console, and GitHub PRs via `gh`).

Remaining refinements: diagonal graph connectors for merges, on-disk rebase
abort/resume for in-app interactive rebase (CLI rebases already resume via the
banner), richer PR review (inline comments), and continued visual polish.
